...
public final class DOMXMLObject extends DOMStructure implements XMLObject {
    ...
    throws MarshalException
    {
        ...
        for (int i = 0; i < length; i++) {
            ...
            content.add(new org.apache.xml.crypto.dom.DOMStructure(child));
        }
        if (content.isEmpty()) {
            this.content = Collections.emptyList();
        } else {
        ...
        if (objElem == null) {
            ...
            for (XMLStructure object : content) {
                if (object instanceof DOMStructure) {
                    ...
                    org.apache.xml.crypto.dom.DOMStructure domObject = 
                        (org.apache.xml.crypto.dom.DOMStructure)object;
    ...
    private boolean equalsContent(List<XMLStructure> otherContent) {
        ...
        for (int i = 0, osize = otherContent.size(); i < osize; i++) {
            ...
            if (oxs instanceof org.apache.xml.crypto.dom.DOMStructure) {
                if (!(xs instanceof org.apache.xml.crypto.dom.DOMStructure)) {
                    return false;
                }
                Node onode = ((org.apache.xml.crypto.dom.DOMStructure)oxs).getNode();
                Node node = ((org.apache.xml.crypto.dom.DOMStructure)xs).getNode();
