 ...
 */
/*
 * $Id: DOMXMLObject.java 1231033 2012-01-13 12:12:12Z coheigea $
 */
package es.gob.afirma.xml.dsig.internal.dom;

import es.gob.afirma.xml.crypto.*;
import es.gob.afirma.xml.crypto.dom.DOMCryptoContext;
import es.gob.afirma.xml.crypto.dsig.*;
...
public final class DOMXMLObject extends DOMStructure implements XMLObject {
    ...
    throws MarshalException
    {
        ...
        for (int i = 0; i < length; i++) {
            ...
            content.add(new es.gob.afirma.xml.crypto.dom.DOMStructure(child));
        }
        if (content.isEmpty()) {
            this.content = Collections.emptyList();
        } else {
        ...
        if (objElem == null) {
            ...
            for (XMLStructure object : content) {
                if (object instanceof DOMStructure) {
                    ...
                    es.gob.afirma.xml.crypto.dom.DOMStructure domObject = 
                        (es.gob.afirma.xml.crypto.dom.DOMStructure)object;
    ...
    private boolean equalsContent(List<XMLStructure> otherContent) {
        ...
        for (int i = 0, osize = otherContent.size(); i < osize; i++) {
            ...
            if (oxs instanceof es.gob.afirma.xml.crypto.dom.DOMStructure) {
                if (!(xs instanceof es.gob.afirma.xml.crypto.dom.DOMStructure)) {
                    return false;
                }
                Node onode = ((es.gob.afirma.xml.crypto.dom.DOMStructure)oxs).getNode();
                Node node = ((es.gob.afirma.xml.crypto.dom.DOMStructure)xs).getNode();
