...
public class RFC3161TSAServiceInvoker {
    ...
    /**
     * Attribute that represents the general properties defined on the configuration file.
     */
     ...
     */
    public RFC3161TSAServiceInvoker() {
     ...
     * <ul>
     * <li>{@link TSAServiceInvokerConstants.RFC3161Protocol#TCP}</li>
     * <li>{@link TSAServiceInvokerConstants.RFC3161Protocol#HTTPS}</li>
     * <li>{@link TSAServiceInvokerConstants.RFC3161Protocol#SSL}</li>
     * </ul>
     ...
     */
    public final byte[ ] generateTimeStampToken(String protocol, String applicationParam, byte[ ] dataParam, String idClient) throws TSAServiceInvokerException {
	// Comprobamos que se han indicado datos de entrada
	if (!GenericUtilsCommons.assertStringValue(protocol)) {
	    throw new TSAServiceInvokerException(Language.getResIntegra(ILogConstantKeys.RTSI_LOG002));
	}
	if (!GenericUtilsCommons.assertStringValue(applicationParam)) {
	...
	properties = new TSAServiceInvokerProperties().getTsaServiceInvokerProperties(idClient, applicationParam);
	if (properties == null) {
	    throw new TSAServiceInvokerException(Language.getResIntegra(ILogConstantKeys.RTSI_LOG001));
	}

	generalProperties = new IntegraProperties().getIntegraProperties(idClient);
	...
	policyOID = properties.getProperty(TSAServiceInvokerConstants.RFC3161_TIMESTAMP_POLICY_OID);
	...
	applicationOID = properties.getProperty(TSAServiceInvokerConstants.RFC3161_APPLICATION_OID);
	...
	String hashAlgorithm = properties.getProperty(TSAServiceInvokerConstants.RFC3161_HASH_ALGORITHM);
	...
	host = properties.getProperty(TSAServiceInvokerConstants.RFC3161_HOST);
	...
	String timeOutStr = properties.getProperty(TSAServiceInvokerConstants.RFC3161_TIMEOUT);
	checkSvcInvokerParams(TSAServiceInvokerConstants.RFC3161_TIMEOUT, timeOutStr);
	try {
	    timeOut = Integer.valueOf(timeOutStr);
	} catch (NumberFormatException e) {
	    throw new TSAServiceInvokerException(Language.getFormatResIntegra(ILogConstantKeys.RTSI_LOG050, new Object[ ] { timeOutStr }), e);
	}
	...
	return generateTimeStampToken(protocol, applicationParam, dataParam, null);
    }
	...
	try {
	    ...
	    String portStr = properties.getProperty(TSAServiceInvokerConstants.RFC3161_PORT_NUMBER);
	    ...
	    UtilsResourcesCommons.safeCloseOutputStream(dataoutputstream);
	}
    }
	...
	try {
	    ...
	    UtilsResourcesCommons.safeCloseInputStream(datainputstream);
	}
	...
	try {
	    ...
	    KeyStore cer = UtilsKeystoreCommons.loadKeystore(trustsorePath, truststorePassword, UtilsKeystoreCommons.JKS);
	    ...
	    String authClientStr = properties.getProperty(TSAServiceInvokerConstants.RFC3161_HTTPS_AUTH);
		...
		// autenticación HTTPS
		String keystorePath = properties.getProperty(TSAServiceInvokerConstants.RFC3161_HTTPS_KEYSTORE_PATH);
		...
		// HTTPS
		String keystoreType = properties.getProperty(TSAServiceInvokerConstants.RFC3161_HTTPS_KEYSTORE_TYPE);
		...
		// almacenada la clave privada a usar para la autenticación
		// HTTPS
		String keystorePassword = properties.getProperty(TSAServiceInvokerConstants.RFC3161_HTTPS_KEYSTORE_PASSWORD);
	    ...
	    UtilsResourcesCommons.safeCloseInputStream(inp);
	...
	String portStr = properties.getProperty(TSAServiceInvokerConstants.RFC3161_HTTPS_PORT_NUMBER);
	Integer portNumber = null;
	...
	String context = properties.getProperty(TSAServiceInvokerConstants.RFC3161_HTTPS_CONTEXT);
	...
	try {
	    ...
	    String portStr = properties.getProperty(TSAServiceInvokerConstants.RFC3161_SSL_PORT_NUMBER);
	    Integer portNumber = null;
	    ...
	    // donde se encuentra almacenada la clave privada a usar para la
	    // autenticación SSL
	    String keystorePath = properties.getProperty(TSAServiceInvokerConstants.RFC3161_SSL_KEYSTORE_PATH);
	    ...
	    // almacenada la clave privada a usar para la autenticación
	    // SSL
	    String keystoreType = properties.getProperty(TSAServiceInvokerConstants.RFC3161_SSL_KEYSTORE_TYPE);
	    ...
	    // almacenada la clave privada a usar para la autenticación
	    // SSL
	    String keystorePassword = properties.getProperty(TSAServiceInvokerConstants.RFC3161_SSL_KEYSTORE_PASSWORD);
	    ...
	    // Cerramos recursos
	    UtilsResourcesCommons.safeCloseSocket(sslsocket);
	    UtilsResourcesCommons.safeCloseOutputStream(os);
	    UtilsResourcesCommons.safeCloseOutputStream(dataoutputstream);
	}
    }
	...
	try {
	    ...
	    if (applicationOID != null && !applicationOID.isEmpty()) {
