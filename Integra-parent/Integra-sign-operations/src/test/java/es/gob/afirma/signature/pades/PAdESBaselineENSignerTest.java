 ...
 * <b>Description:</b><p>Class that defines tests for {@link PAdESBaselineENSigner}.</p>
...
/**
 * <p>Class that defines tests for {@link PAdESBaselineENSigner}.</p>
 * <b>Project:</b><p>Library for the integration with the services of @Firma, eVisor and TS@.</p>
 * @version 1.2, 14/03/2017.
 */
public class PAdESBaselineENSignerTest extends AbstractSignatureTest {
    ...
    /**
     * Test for methods {@link PAdESBaselineENSigner#sign(byte[], String, String, PrivateKeyEntry, java.util.Properties, boolean, String, String)},
     * and {@link PAdESBaselineENSigner#verifySignature(byte[])}.
     */
	...
	PAdESBaselineENSigner signer = new PAdESBaselineENSigner();
    ...
    /**
     * Test for method {@link PAdESBaselineENSigner#getSignedData(byte[])}.
     */
    public final void testGetSignedData() {

	byte[ ] signature = UtilsFileSystemCommons.readFile("signatures/PDF/PDF.pdf", true);
	PAdESBaselineENSigner pbs = new PAdESBaselineENSigner();
    ...
    /**
     * Test for method {@link PAdESBaselineENSigner#verifySignature(byte[])} with a valid PAdES B-B-Level signature.
     */
    public final void testVerifySignatureBBLevel() {

	byte[ ] signaturePdf = UtilsFileSystemCommons.readFile("signatures/PAdES_BB_Level_OK.pdf", true);
	
	PAdESBaselineENSigner pbs = new PAdESBaselineENSigner();
	PDFValidationResult vr = pbs.verifySignature(signaturePdf);
	Assert.assertTrue(vr.isCorrect());
    }
    
    /**
     * Test for method {@link PAdESBaselineENSigner#verifySignature(byte[])} with a valid PAdES B-B-Level signature with
     * a "commitment-type-indication" and "signature-policy-id" atributtes.
     */
    public final void testVerifySignatureBBLevelCommintmentWithoutPolicy() {

	byte[ ] signaturePdf = UtilsFileSystemCommons.readFile("signatures/PAdES_BB_Level_OK_CommitmentWithoutPolicy.pdf", true);
	
	PAdESBaselineTSSigner pbs = new PAdESBaselineTSSigner();
	PDFValidationResult vr = pbs.verifySignature(signaturePdf);
	Assert.assertTrue(vr.isCorrect());
    }
    ...
    /**
     * Test for method {@link PAdESBaselineENSigner#verifySignature(byte[])} with a valid PAdES B-B-Level signature with
     * a "commitment-type-indication" and without a "signature-policy-id" atributtes (PAdES EPES
     */
    public final void testVerifySignatureBBLevelWithCommintmentAndPolicy() {

	byte[ ] signaturePdf = UtilsFileSystemCommons.readFile("signatures/PAdES_BB_Level_OK_CommitmentAndPolicy.pdf", true);
    ...
    /**
     * Test for method {@link PAdESBaselineENSigner#verifySignature(byte[])} with a invalid PAdES B-B-Level signature with
     * a "commitment-type-indication" attributte and the "reason" dictionary key.
     */
    public final void testVerifySignatureBBLevelWithCommintmentAndReason() {

	byte[ ] signaturePdf = UtilsFileSystemCommons.readFile("signatures/PAdES_BB_Level_CommitmentAndReason.pdf", true);
	
	PAdESBaselineENSigner pbs = new PAdESBaselineENSigner();
	PDFValidationResult vr = pbs.verifySignature(signaturePdf);
	Assert.assertFalse(vr.isCorrect());
	System.out.println(vr.getErrorMsg());
    }
    
    /**
     * Test for method {@link PAdESBaselineENSigner#verifySignature(byte[])} with a invalid PAdES B-B-Level signature with
     * a "signature-policy-id" attributte and the "reason" dictionary key.
     */
    public final void testVerifySignatureBBLevelWithPolicyAndReason() {

	byte[ ] signaturePdf = UtilsFileSystemCommons.readFile("signatures/PAdES_BB_Level_PolicyAndReason.pdf", true);
