...
import net.java.xades.security.xml.XAdES.SignerRoleV2;
import net.java.xades.security.xml.XAdES.SignerRoleV2Impl;
import net.java.xades.security.xml.XAdES.XAdES;
import net.java.xades.security.xml.XAdES.XAdES_B_B_Level;
 ...
 * @version 1.9, 16/04/2020.
 */
public final class XAdESBaselineENSigner implements Signer {

    /**
     * Attribute that represents the object that manages the log of the class.
     */
    private static final Logger LOGGER = IntegraLogger.getInstance().getLogger(XAdESBaselineENSigner.class);
	...
	AccessController.doPrivileged(new java.security.PrivilegedAction<Void>() {
		...
		try {
		    ...
		    LOGGER.error(Language.getResIntegra(ILogConstantKeys.XBS2_LOG001), e);
		}
		return null;
	    }
	});
     ...
     * The allowed values are:
     * <ul>
     * <li>{@link es.gob.afirma.signature.ISignatureFormatDetector#FORMAT_XADES_B_B_LEVEL}</li>
     * </ul>
	...
	XAdES_B_B_Level xades = generateXAdESElement(privateKey, optionalParams, docSignature.getDocumentElement());
	...
	if (signaturePolicyID != null) {
		...
		LOGGER.debug(Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG025, new Object[ ] { IIntegraConstants.DEFAULT_PROPERTIES_FILE, signaturePolicyID }));
		...
		// firma asociada a las firmas XML
		...
		// para XAdES no sea nulo ni vacío
		if (!GenericUtilsCommons.assertStringValue(policyID)) {
		    String errorMsg = Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG026, new Object[ ] { signaturePolicyID, IIntegraConstants.DEFAULT_PROPERTIES_FILE });
		    LOGGER.warn(errorMsg);
		    mustContainSignaturePolicy = false;
		} else {
		    LOGGER.info(Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG042, new Object[ ] { signaturePolicyID, IIntegraConstants.DEFAULT_PROPERTIES_FILE, policyID }));
		}
	    }
	...
	if (dsSignature == null) {
	    String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG031);
	...
	if (mustContainSignaturePolicy) {
		...
		String errorMsg = Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG034, new Object[ ] { e.getMessage() });
		LOGGER.error(errorMsg, e);
		throw new SigningException(errorMsg, e);
	    }
	}

	// Informamos de que hemos generado la firma
	...
	LOGGER.info(Language.getResIntegra(ILogConstantKeys.XBS2_LOG035));
	...
	LOGGER.info(Language.getResIntegra(ILogConstantKeys.XBS2_LOG002));
	try {
	    ...
	    String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG036);
	    LOGGER.error(errorMsg, e);
	    throw new SigningException(errorMsg, e);
	    ...
	    LOGGER.info(Language.getResIntegra(ILogConstantKeys.XBS2_LOG003));
	}
    }
	...
	if (applicationID == null || applicationID.trim().isEmpty()) {
	    ...
	    String errorMsg = Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG048, new Object[ ] { propertiesName });
	    LOGGER.error(errorMsg);
	    throw new SigningException(errorMsg);
	}
    }
	...
	if (tsaCommunicationMode == null || tsaCommunicationMode.trim().isEmpty()) {
	    String propertiesName = IIntegraConstants.PROPERTIES_FILE;
	    String errorMsg = Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG049, new Object[ ] { propertiesName });
	    LOGGER.error(errorMsg);
	    throw new SigningException(errorMsg);
	}
    }
	...
	if (timestampType == null || timestampType.trim().isEmpty()) {
	    String propertiesName = IIntegraConstants.PROPERTIES_FILE;
	    String errorMsg = Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG050, new Object[ ] { propertiesName });
	    LOGGER.error(errorMsg);
	    throw new SigningException(errorMsg);
	}
    }
	...
	if (timestampType.equals(IUtilsTimestamp.TIMESTAMP_TYPE_ASN1)) {
	    ...
	    String propertiesName = IIntegraConstants.PROPERTIES_FILE;
	    String errorMsg = Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG033, new Object[ ] { timestampType, propertiesName });
	    LOGGER.error(errorMsg);
	    throw new SigningException(errorMsg);
	}
    }
	...
	if (includeTimestamp) {
		...
		try {
		    ...
		    throw new SigningException(Language.getResIntegra(ILogConstantKeys.XBS2_LOG015), e);
		}
     ...
     * @throws SigningException If the method fails.
     */
    private void addSignPolicy(boolean addSignaturePolicy, String uriSignAlgorithm, String policyID, Properties policyProperties, Properties extraParams, XadesWithExplicitPolicy xades, String signType, String idClient) throws SigningException {
	if (addSignaturePolicy) {
		...
		String errorMsg = Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG027, new Object[ ] { uriSignAlgorithm, policyID, IIntegraConstants.DEFAULT_PROPERTIES_FILE });
		LOGGER.error(errorMsg);
		throw new SigningException(errorMsg);
	    }
		...
		String errorMsg = Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG028, new Object[ ] { digestAlgorithmRef, policyID, IIntegraConstants.DEFAULT_PROPERTIES_FILE });
		LOGGER.error(errorMsg);
		throw new SigningException(errorMsg);
	    }
		...
		String errorMsg = Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG029, new Object[ ] { signingMode, policyID, IIntegraConstants.DEFAULT_PROPERTIES_FILE });
		LOGGER.error(errorMsg);
		throw new SigningException(errorMsg);
	    }
		...
		String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG030);
		LOGGER.error(errorMsg, e);
		throw new SigningException(errorMsg, e);
	    }
	}
	...
	try {
	    ...
	    String errorMsg = Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG020, new Object[ ] { digestAlgorithmRef });
	    LOGGER.error(errorMsg, e);
	    throw new SigningException(errorMsg, e);
	}
	return digestMethod;
	...
	try {
	    ...
	    String errorMsg = Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG021, new Object[ ] { CanonicalizationMethod.EXCLUSIVE });
	    LOGGER.error(errorMsg, e);
	    throw new SigningException(errorMsg, e);
	}
    }
	...
	if (signatureFormat.equals(SIGN_FORMAT_XADES_ENVELOPING)) {
		...
		String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG022);
		LOGGER.error(errorMsg, e);
		throw new SigningException(errorMsg, e);
	    }
	...
	try {
	    ...
	    String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG052);
	    LOGGER.error(errorMsg);
	    throw new SigningException(errorMsg);
	...
	try {
		...
		for (ReferenceDataBaseline ref: refAndDataFormatObjList) {
			...
			String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG023);
			LOGGER.error(errorMsg);
			throw new SigningException(errorMsg);
		    }
		}
	    ...
	    String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG024);
	    LOGGER.error(errorMsg, e);
	    throw new SigningException(errorMsg, e);
     ...
     */
    @SuppressWarnings("unchecked")
    private XAdES_B_B_Level generateXAdESElement(PrivateKeyEntry privateKey, Properties extraParams, Element element) {
	// Generamos el objeto que generará la firma
	XAdES_B_B_Level xades = (XAdES_B_B_Level) XAdES.newInstance(XAdES.B_B_LEVEL, IXMLConstants.XADES_1_3_2_NAMESPACE, IXMLConstants.XADES_PREFIX, IXMLConstants.DS_PREFIX, digestAlgorithmRef, element);
	...
	xades.setSigningCertificateV2((X509Certificate) privateKey.getCertificate(), null);
	...
	if (GenericUtilsCommons.assertStringValue(claimedRoles)) {
	    ...
	    SignerRoleV2 signerRole = new SignerRoleV2Impl();
		...
		LOGGER.debug(Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG019, new Object[ ] { claimedRole }));
		signerRole.addClaimedRole(claimedRole);
	    }
	    xades.setSignerRoleV2(signerRole);
	}
	...
	if (!extraParams.containsKey(SignatureProperties.XADES_DATA_FORMAT_DESCRIPTION_PROP) || extraParams.getProperty(SignatureProperties.XADES_DATA_FORMAT_DESCRIPTION_PROP).isEmpty()) {
		...
		List<ReferenceDataBaseline> manifestRef = (List<ReferenceDataBaseline>) extraParams.get(SignatureConstants.MF_REFERENCES_PROPERTYNAME);
	...
	if (!extraParams.containsKey(SignatureProperties.XADES_DATA_FORMAT_MIME_PROP) || extraParams.getProperty(SignatureProperties.XADES_DATA_FORMAT_MIME_PROP).isEmpty()) {
		...
		// Indicamos el Mime-Type de los datos a firmar
	...
	try {
	    ...
	    String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG014);
	    LOGGER.error(errorMsg, e);
	    throw new SigningException(errorMsg, e);
	}
    }
	...
	if (!SIGN_FORMAT_XADES_EXTERNALLY_DETACHED.equals(signatureFormat)) {
		...
		String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG012);
		LOGGER.error(errorMsg, e);
		throw new SigningException(errorMsg, e);
	    }
	}
	...
	if (signatureFormat.equals(SIGN_FORMAT_XADES_ENVELOPED)) {
	    String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG013);
	    LOGGER.error(errorMsg);
	    throw new SigningException(errorMsg);
	}
	...
	if (signatureFormat == null) {
	    String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG010);
	    LOGGER.error(errorMsg);
	...
	if (!SignatureConstants.SUPPORTED_XADES_SIGN_FORMAT.contains(signatureFormat)) {
	    String msg = Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG011, new Object[ ] { signatureFormat });
	    LOGGER.error(msg);
	...
	if (!signatureFormat.equals(SignatureConstants.SIGN_FORMAT_XADES_EXTERNALLY_DETACHED) && data == null) {
	    String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG004);
	    LOGGER.error(errorMsg);
	    throw new IllegalArgumentException(errorMsg);
	}
    }
	...
	if (privateKey == null) {
	    String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG009);
	    LOGGER.error(errorMsg);
	    throw new IllegalArgumentException(errorMsg);
	}
    }
	...
	if (signatureForm == null) {
	    String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG007);
	    LOGGER.error(errorMsg);
	    throw new IllegalArgumentException(errorMsg);
	}
	...
	// soportado.
	if (!signatureForm.equals(ISignatureFormatDetector.FORMAT_XADES_B_B_LEVEL)) {
	    String msg = Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG008, new Object[ ] { signatureForm });
	    LOGGER.error(msg);
	    throw new IllegalArgumentException(msg);
	}
    }
	...
	if (signatureAlgorithm == null) {
	    String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG005);
	    LOGGER.error(errorMsg);
	    throw new IllegalArgumentException(errorMsg);
	}
	...
	if (!SignatureConstants.SIGN_ALGORITHMS_SUPPORT_CADES.containsKey(signatureAlgorithm)) {
	    String msg = Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG006, new Object[ ] { signatureAlgorithm });
	    LOGGER.error(msg);
	    throw new IllegalArgumentException(msg);
	}
    }
	...
	if (signature == null) {
	    String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG039);
	    LOGGER.error(errorMsg);
	    throw new IllegalArgumentException(errorMsg);
	}
    }
	...
	LOGGER.info(Language.getResIntegra(ILogConstantKeys.XBS2_LOG037));
	try {
	    ...
	    LOGGER.debug(Language.getFormatResIntegra(ILogConstantKeys.XBS2_LOG040, new Object[ ] { signType }));
	    ...
	    byte[ ] result = xmlResult.getBytes(SignatureConstants.UTF8_ENCODING);
	    LOGGER.info(Language.getResIntegra(ILogConstantKeys.XBS2_LOG041));
	    ...
	    LOGGER.info(Language.getResIntegra(ILogConstantKeys.XBS2_LOG038));
	}
    }
	...
	try {
	    ...
	    XAdES_B_B_Level xades = generateXAdESElement(privateKey, optionalParams, eSign.getDocumentElement());
	...
	LOGGER.info(Language.getResIntegra(ILogConstantKeys.XBS2_LOG043));
	try {
	    // Comprobamos que se han indicado la firma a actualizar
	    checkInputSignature(signature);
	    ...
	    byte[ ] result = xmlResult.getBytes(SignatureConstants.UTF8_ENCODING);
	    LOGGER.info(Language.getResIntegra(ILogConstantKeys.XBS2_LOG045));
	    ...
	    LOGGER.info(Language.getResIntegra(ILogConstantKeys.XBS2_LOG044));
	}
    }
	...
	for (Element signElement: listSignaturesToCounterSign) {
	    ...
	    XAdES_B_B_Level xades = generateXAdESElement(privateKey, optionalParams, counterSignature);
	...
	LOGGER.info(Language.getResIntegra(ILogConstantKeys.XBS2_LOG046));
	try {
	    // Comprobamos que se han indicado la firma a actualizar
	    checkInputSignature(signature);
		...
		String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG017);
		LOGGER.error(errorMsg, e);
		throw new SigningException(errorMsg, e);
	    }

	} finally {
	    LOGGER.info(Language.getResIntegra(ILogConstantKeys.XBS2_LOG047));
	}
    }
	...
	throw new SigningException(Language.getResIntegra(ILogConstantKeys.XBS2_LOG051));
    }
	...
	LOGGER.info(Language.getResIntegra(ILogConstantKeys.XBS2_LOG016));
	...
	try {
	    ...
	    GenericUtilsCommons.checkInputParameterIsNotNull(xmlDocument, Language.getResIntegra(ILogConstantKeys.XBS2_LOG053));
	     ...
	     */
	    String signingMode = checkSignatureIntegrity(xmlDocument, validationResult, listSigners);
		...
		// Si el firmante no tiene formato Baseline,
		// derivamos la validación del
		// firmante a la clase asociada a firmas no Baseline
		if (!SignatureFormatDetectorXades.isXAdESBaseline(signerFormat)) {
		...
		// Si el firmante tiene un formato Baseline
		// conforme a la
		// especificacion técnica, derivamos la validación
		// del firmante a su clase correspondiente
		else if (SignatureFormatDetectorXades.isXAdESBaselineTS(signerFormat)) {
		    XadesSigner xadesBaselineTsSigner = new XadesSigner();
		    signerValidationResult = xadesBaselineTsSigner.validateSigner(signingMode, signerInfo, validationResult, idClient, signerFormat, false);
		}
		
		// En cualquier otro caso, validamos directamente
		// al firmante
		else {
	    ...
	    LOGGER.info(Language.getResIntegra(ILogConstantKeys.XBS2_LOG055));
	    ...
	    LOGGER.info(Language.getResIntegra(ILogConstantKeys.XBS2_LOG054));
	} finally {
	    LOGGER.info(Language.getResIntegra(ILogConstantKeys.XBS2_LOG032));
	}
     ...
     */
    private String checkSignatureIntegrity(byte[ ] xmlDocument, ValidationResult validationResult, List<XAdESSignerInfo> listSigners) throws SigningException {
	...
	if (listSignersFound.isEmpty()) {
	    String errorMsg = Language.getResIntegra(ILogConstantKeys.XBS2_LOG018);
	    LOGGER.error(errorMsg);
	    validationResult.setIntegrallyCorrect(false);
	...
	// Determinamos a partir del formato si el firmante es Baseline y si se
	// ajusta al estandar europeo
	...
	try {
	    ...
	    // el elemento firmado xades:SigningCertificateV2 se corresponde con
	     ...
	     * > Se comprobará que existe una referencia que apunta al elemento xades:SignedProperties.
	     */
	    validateSignatureCore(signerValidationResult, signerInfo, validationResult, signedSignaturePropertiesElement, signedPropertiesElement, isBaseline, isBaselineEN, signedFile, signedFileName, isCounterSignature);
	    ...
	    // También se comprobará que, si se define el atributo no firmado
	    // xades141:SignaturePolicyStore, también se debe haber definido
	    // xades:SignaturePolicyIdentifier y el identificador de ambos
	    // atributos debe coincidir.
	...
	// Si no tiene definida una politica de firma, no puede incluir un SignaturePolicyStore
	if (SignatureFormatDetectorXades.hasSignaturePolicyStore(signerInfo.getElementSignature())) {

	    // Si no se ha hecho ya, instanciamos el objeto con la informacion de validacion y lo
	    // agregamos
	    // a la lista de validaciones del firmante/contra-firmante
	    ...
	    try {
		// Validamos de la política de firma almacenada:
		//  - Que no exista si no existe una política de firma declarada.
		//  - Que el id de la política almacenada sea el mismo que el de la política declarada.
		//  - Que se encuentre un documento almacenado o referenciado.
		//  - Que el calculo del hash de la política almacenada coincida con el declarado en la política.
		SignaturePolicyManager.validateXAdESSignaturePolicyStore(signerInfo.getElementSignature());

	    } catch (SignaturePolicyException e) {
		// Establecemos, a nivel general, el error asociado a la
		// validación
		// de la integridad de la politica de firma almacenada, si es que no se indicó
		// previamente
		if (validationResult.getErrorMsg() == null) {
		    validationResult.setErrorMsg(e.getMessage());
		}
	...
	// Habiendo terminado el proceso, si tenemos informacion de validacion, indicamos que 
     ...
     * @throws SigningException If the validation fails.
     */
    private void validateSignatureCore(SignerValidationResult signerValidationResult, XAdESSignerInfo signerInfo, ValidationResult validationResult, Element signedSignaturePropertiesElement, Element signedPropertiesElement, boolean isBaseline, boolean isBaselineEN, byte[ ] signedFile, String signedFileName, boolean isCounterSignature) throws SigningException {
	// Instanciamos el objeto que ofrece información sobre la validación
	// llevada a cabo
	ValidationInfo validationInfo = new ValidationInfo();
	...
	try {
	    ...
	    UtilsSignatureOp.validateXAdESSignatureCore(signerInfo.getQualifyingPropertiesElement(), signerInfo.getId(), signerInfo.getElementSignature(), signerInfo.getSignature(), signedFile, signedFileName, signerInfo.getSigningCertificate(), signedSignaturePropertiesElement, signedPropertiesElement, isBaseline, isBaselineEN, isCounterSignature);
	...
	if (listCounterSignerInfo != null && !listCounterSignerInfo.isEmpty()) {
		...
		// Si el contra-firmante tiene un formato Baseline
		// conforme a la
		// especificacion técnica, derivamos la validación
		// del firmante a su clase correspondiente
		else if (SignatureFormatDetectorXades.isXAdESBaselineTS(signerFormat)) {
