...
/**
 * <p>Class that manages the generation, validation and upgrade of CAdES Baseline European Standard
 * signatures.</p>
 * <b>Project:</b><p>Library for the integration with the services of @Firma, eVisor and TS@.</p>
 * @version 1.7, 13/04/2020.
 */
public final class CAdESBaselineENSigner implements Signer {

    /**
     *  Attribute that represents the object that manages the log of the class.
     */
    private static final Logger LOGGER = IntegraLogger.getInstance().getLogger(CAdESBaselineENSigner.class);
    ...
    /**
     * Method that validates the signers of a CAdES Baseline European Standard signature.
    ...
    /**
     * Method that validates a CAdES Baseline European Standard signature.
     * @param eSignature Parameter that represents the signature to validate.
	...
	try {
		...
		// Si el firmante tiene formato Baseline European Standard, nos mantenemos en esta
		// clase. En otro caso, derivamos la validación del firmante a
		// la clase del formato de firma correspondiente
		SignerValidationResult signerValidationResult = null;
		if (signerIsBaselineEN(signerFormat)) {
		...
		} else if (signerIsBaselineTS(signerFormat)) {
	...
	if (signatureFormat.equals(ISignatureFormatDetector.FORMAT_CADES_B_B_LEVEL)
		|| signatureFormat.equals(ISignatureFormatDetector.FORMAT_CADES_B_T_LEVEL)
		|| signatureFormat.equals(ISignatureFormatDetector.FORMAT_CADES_B_LT_LEVEL)
		|| signatureFormat.equals(ISignatureFormatDetector.FORMAT_CADES_B_LTA_LEVEL)) {
	...
	if (SignatureFormatDetectorCadesPades.hasSignaturePolicyStore(signerInfo.getSignerInformation())) {
		...
		SignaturePolicyManager.validateCAdESSignaturePolicyStore(signerInfo.getSignerInformation());
	...
	if (listCounterSignerInfo != null && !listCounterSignerInfo.isEmpty()) {
		...
		if (signerIsBaselineEN(signerFormat)) {
		    ...
		    // Obtenemos la información de validación asociada al
		    // contra-firmante
		    counterSignerValidationResult = cadesBaselineTSSigner.validateSigner(signedData, counterSignerInfo, validationResult, idClient, true, signerFormat);
		} else { 
		    CadesSigner cadesSigner = new CadesSigner();
