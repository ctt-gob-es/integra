 ...
 * @version 1.3, 13/04/2020.
 */
public final class PAdESBaselineENSigner implements Signer {

    /**
     * Attribute that represents the object that manages the log of the class.
     */
    private static final Logger LOGGER = IntegraLogger.getInstance().getLogger(PAdESBaselineENSigner.class);

    /**
     * Constructor method for the class PAdESBaselineSigner.java.
     */
    public PAdESBaselineENSigner() {
	...
	try {
	    ...
	    // informamos de que sólo puede ser implicita
	    ...
	    addPropertyToDictionary(externalParams, signDictionary, signaturePolicyID);

	    // Incluimos el diccionario de firma
	    ...
	    // Incluimos la firma CAdES en el diccionario de firma
     ...
     * @param signPolicyId signature policy identifier
     */
    private void addPropertyToDictionary(Properties externalParams, PdfSignature signDictionary, String signPolicyId) {
	
	// Razón de firma. Sólo se agregará a las firmas B-B-Level cuando no se defina
	// una política de firma
	String reason = externalParams.getProperty(SignatureProperties.PADES_REASON_PROP);
	if (!GenericUtilsCommons.assertStringValue(signPolicyId)
		&& GenericUtilsCommons.assertStringValue(reason)) {
	...
	for (PDFSignatureDictionary pdfSignatureDictionary: listSignatureDictionaries) {
		...
		// Si el formato de firma es PDF (no contiene firmas) no realizamos ninguna
		// validación sobre
		// el documento PDF ni sobre el firmante
		...
		// Si el formato del diccionario de firma es el de una firma BASIC, BES o EPES 
		else if (isAdvancedFormat(signatureFormat)) {
		    PadesSigner.validateSignatureDictionaries(
		                                              listSignatureDictionaries,
		                                              listPDFSignatureDictionariesValidationResult,
		                                              af,
		                                              validationResult,
		                                              listTimestampDictionaries,
		                                              pdfDocument,
		                                              idClient);
		}
		// Si el formato del diccionario de firma es el de una firma Baseline acorde a la especificación técnica
		else if (isBaselineTSFormat(signatureFormat)) {
		    PAdESBaselineTSSigner.validateSignatureDictionaries(
		                                                        listSignatureDictionaries,
		                                                        listPDFSignatureDictionariesValidationResult,
		                                                        af,
		                                                        validationResult,
		...
		// Si el formato del diccionario de firma es el de una firma Baseline acorde al estándar europeo
		else if (isBaselineENFormat(signatureFormat)) {
		     ...
		     * > Si el primer firmante de la firma CAdES contenida en el diccionario de firma posee el atributo firmado signature-policy-id entonces no deberá poseer la
		     * clave /Reason del diccionario de firma.
		     * > Si el primer firmante de la firma CAdES contenida en el diccionario de firma posee el atributo firmado commitment-type-indication entonces no deberá poseer la
		     * clave /Reason del diccionario de firma.
		    ...
		    // Si hay declarados ellos de tiempo, se validan
		    if (listTimestampDictionaries != null && listTimestampDictionaries.size() > 0) {
			/*
	...
	try {
	    ...
	    // PAdES Baseline
	    hasSignaturePolicyId = UtilsSignatureOp.validatePAdESBaselineENStructurally(pdfSignatureDictionary, pdfDocument, signedData);
    ...
    private static boolean isAdvancedFormat(String signatureFormat) {
	return signatureFormat == ISignatureFormatDetector.FORMAT_PADES_BASIC
		|| signatureFormat == ISignatureFormatDetector.FORMAT_PADES_BES
		|| signatureFormat == ISignatureFormatDetector.FORMAT_PADES_EPES
		|| signatureFormat == ISignatureFormatDetector.FORMAT_PADES_LTV;
    }

    private static boolean isBaselineTSFormat(String signatureFormat) {
	return signatureFormat == ISignatureFormatDetector.FORMAT_PADES_B_LEVEL
		|| signatureFormat == ISignatureFormatDetector.FORMAT_PADES_T_LEVEL
		|| signatureFormat == ISignatureFormatDetector.FORMAT_PADES_LT_LEVEL
		|| signatureFormat == ISignatureFormatDetector.FORMAT_PADES_LTA_LEVEL;
    }
    
    private static boolean isBaselineENFormat(String signatureFormat) {
	return signatureFormat == ISignatureFormatDetector.FORMAT_PADES_B_B_LEVEL
		|| signatureFormat == ISignatureFormatDetector.FORMAT_PADES_B_T_LEVEL
		|| signatureFormat == ISignatureFormatDetector.FORMAT_PADES_B_LT_LEVEL
		|| signatureFormat == ISignatureFormatDetector.FORMAT_PADES_B_LTA_LEVEL;
    }
}
