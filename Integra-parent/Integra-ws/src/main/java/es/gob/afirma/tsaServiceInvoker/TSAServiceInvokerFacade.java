...
public final class TSAServiceInvokerFacade {
     ...
     */
    private static final Logger LOGGER = IntegraLogger.getInstance().getLogger(TSAServiceInvokerFacade.class);
	...
	return invokeService(xmlInput, service, applicationName, null);
    }
     ...
     * @throws TSAServiceInvokerException if the method fails.
     */
    public String invokeService(String xmlInput, String service, String applicationName, String idClient) throws TSAServiceInvokerException {
	// Comprobamos que los parámetros de entrada no son nulos
	if (!GenericUtilsCommons.assertStringValue(xmlInput) || !GenericUtilsCommons.assertStringValue(service) || !GenericUtilsCommons.assertStringValue(applicationName)) {
	...
	try {
	    ...
	    tsaInvoker = TSAServiceInvokerFactory.getTSAServiceInvoker(applicationName, service, idClient);
	    res = tsaInvoker.invokeService(serviceInParam, idClient);
	} catch (Exception e) {
	    LOGGER.error(e);
