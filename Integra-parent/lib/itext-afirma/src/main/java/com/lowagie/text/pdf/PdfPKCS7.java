...
public class PdfPKCS7 {
        ...
        try {
            for (Enumeration aliases = keystore.aliases(); aliases.hasMoreElements();) {
                try {
                    ...
                    ContentVerifierProvider verifier = verifierBuilder.build(certStoreX509.getPublicKey());
                    if (ocsp.isSignatureValid(verifier))
                        return true;
                }
                catch (Exception ex) {
                }
        ...
        try {
            for (Enumeration aliases = keystore.aliases(); aliases.hasMoreElements();) {
                try {
                    ...
                    SignerInformationVerifier verifier = verifierBuilder.build(certStoreX509);
                    ts.validate(verifier);
    ...
    public byte[] getAuthenticatedAttributeBytes(byte secondDigest[], Calendar signingTime, byte[] ocsp) {
        try {
            return getAuthenticatedAttributeSet(secondDigest, signingTime, ocsp).getEncoded();
        }
        catch (Exception e) {
            throw new ExceptionConverter(e);
        }
