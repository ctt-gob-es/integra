    ...
    */
	...
	if (checkIfStatusDeterminationApproachIsDelinquentOrEquivalent(tsl.getSchemeInformation().getStatusDeterminationApproach().toString())) {
		...
		if (resultQC.getInfoQcResult().isCertificateDetected() || resultQC.getInfoQcResult().isTspServiceTSADetected()) {
		    ...
		    // asigna el certificado emisor obtenido al resultado.
	...
	if (tspList != null && !tspList.isEmpty()) {
		...
		if (isTsaCertificate) {
		    LOGGER.info(Language.getResIntegraTsl(ILogTslConstant.ATV_LOG081));
		    ...
		    LOGGER.info(Language.getResIntegraTsl(ILogTslConstant.ATV_LOG082));
		}
		endProc = Boolean.TRUE;
		resultQC.setEndProcedure(endProc);
	...
	if (tspServiceList != null && !tspServiceList.isEmpty()) {
		...
		if (checkIfTSPServiceTypeIsCAQC(tspServiceType)) {
			...
			if (siResult.isError()) {
			    ...
			    LOGGER.info(Language.getFormatResIntegraTsl(ILogTslConstant.ATV_LOG083, new Object[ ] { validationDate }));
			} else {
