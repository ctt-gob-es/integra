...
/**
 * <p>
 * Class that represents a TSL Data Checker of TSL implementation as the ETSI TS
 * 119612 2.1.1 specification.
 * </p>
 * <b>Project:</b>
 * <p>
...
public class TSLChecker extends ATSLChecker {
	...
	/**
	 * Constant attribute that represents the start date form which applies this
	 * specification.
	 */
	 ...
	 * 
	 * @param tslObject
	 *            TSL object representation that will be checked.
	 */
	...
	protected void checkTSLTagValue() throws TSLMalformedException {
		...
		if (!ITSLCommonURIs.TSL_TAG_19612.equalsIgnoreCase(tslTagUri.toString())) {
			...
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG001,
					new Object[] { ITSLElementsAndAttributes.ATTRIBUTE_TSL_TAG }));
	...
	protected void checkSchemeInformationTSLVersionIdentifierValue() throws TSLMalformedException {
		if (getTSLObject().getSchemeInformation().getTslVersionIdentifier() != NumberConstants.INT_5
			...
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG003,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_TSL_VERSION_IDENTIFIER }));
		}
	...
	protected void checkSchemeInformationTSLTypeValue() throws TSLMalformedException {
		...
		if (tslTypeUri == null) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG002,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_TSL_TYPE }));
		}
		...
		valueOK = valueOK || isTSLTypeNonEUCountry(tslTypeUriString)
				|| isTSLTypeNonEUCountryListOfTheList(tslTypeUriString);
		if (!valueOK) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG003,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_TSL_TYPE }));
		}
	 ...
	 * 
	 * @param tslType
	 *            String that represents the TSL Type URI.
	 * @return <code>true</code> if the TSL type URI is valid and it is for an
	 *         EU member, otherwise <code>false</code>.
	 */
	 ...
	 * @param tslType
	 *            String that represents the TSL Type URI.
	 * @return <code>true</code> if the TSL type URI is valid and it is EU list
	 *         of the lists, otherwise <code>false</code>.
	 */
	 ...
	 * @param tslType
	 *            String that represents the TSL Type URI.
	 * @return <code>true</code> if the TSL type URI is valid and it is for non
	 *         EU country, otherwise <code>false</code>.
	 */
	private boolean isTSLTypeNonEUCountry(String tslType) {

		if (tslType.startsWith(ITSLCommonURIs.TSL_TYPE_NONEUGENERIC_PREFFIX)
				&& tslType.endsWith(ITSLCommonURIs.TSL_TYPE_NONEUGENERIC_SUFFIX)) {
			return UtilsCountryLanguage
					.checkCountryCode(tslType.substring(ITSLCommonURIs.TSL_TYPE_NONEUGENERIC_PREFFIX.length(),
							tslType.length() - ITSLCommonURIs.TSL_TYPE_NONEUGENERIC_SUFFIX.length()));
		}
		return false;
	...
	private boolean isTSLTypeNonEUCountryListOfTheList(String tslType) {
		...
		if (tslType.startsWith(ITSLCommonURIs.TSL_TYPE_NONEULISTOFTHELISTS_PREFFIX)
				&& tslType.endsWith(ITSLCommonURIs.TSL_TYPE_NONEULISTOFTHELISTS_PREFFIX)) {
			return UtilsCountryLanguage
					.checkCountryCode(tslType.substring(ITSLCommonURIs.TSL_TYPE_NONEULISTOFTHELISTS_PREFFIX.length(),
							tslType.length() - ITSLCommonURIs.TSL_TYPE_NONEULISTOFTHELISTS_PREFFIX.length()));
		}
		return false;
	...
	protected void checkSchemeInformationOperatorNameValue() throws TSLMalformedException {
		...
		List<String> sionvList = getTSLObject().getSchemeInformation()
				.getSchemeOperatorNameInLanguage(Locale.UK.getLanguage());
		if (sionvList == null || sionvList.isEmpty()) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG005,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_SCHEME_OPERATOR_NAME }));
		}
	...
	protected void checkAddressValue(Address address) throws TSLMalformedException {
		...
		if (paList == null) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG005,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_SCHEME_OPERATOR_ADDRESS }));
		}
	...
	protected void checkSchemeInformationNameValue() throws TSLMalformedException {
		...
		if (nameEnglish == null) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG005,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_SCHEME_NAME }));
		}
		...
		if (names != null && !names.isEmpty()) {
			...
			for (String name : namesCollection) {
				...
				if (UtilsStringChar.isNullOrEmptyTrim(name) || name.length() < 2
						|| !name.startsWith(getTSLObject().getSchemeInformation().getSchemeTerritory())) {
					throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG006,
							new Object[] { ITSLElementsAndAttributes.ELEMENT_SCHEME_NAME }));
				}
	...
	protected void checkSchemeInformationURIValue() throws TSLMalformedException {
		...
		// Debe estar definido al menos en inglés.
		List<URI> uriList = getTSLObject().getSchemeInformation()
				.getSchemeInformationURIinLanguage(Locale.UK.getLanguage());
		if (uriList == null || uriList.isEmpty()) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG005,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_SCHEME_INFORMATION_URI }));
		}
	...
	protected void checkSchemeInformationStatusDeterminationApproachValue() throws TSLMalformedException {
		...
		if (isTSLTypeEUGeneric(tslType) || isTSLTypeEUlistOfTheList(tslType)) {
			...
			if (sda.startsWith(ITSLCommonURIs.TSL_STATUSDETAPPROACH_NONEU_PREFFIX)
					&& sda.endsWith(ITSLCommonURIs.TSL_STATUSDETAPPROACH_NONEU_SUFFIX)) {

				String country = sda.substring(ITSLCommonURIs.TSL_STATUSDETAPPROACH_NONEU_PREFFIX.length(),
						sda.length() - ITSLCommonURIs.TSL_STATUSDETAPPROACH_NONEU_SUFFIX.length());
		...
		if (!valueOK) {
			...
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG003,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_STATUS_DETERMINATION_APPROACH }));
	...
	protected void checkSchemeInformationTypeCommunityRulesDefined() throws TSLMalformedException {
		...
		if (!getTSLObject().getSchemeInformation().isThereSomeSchemeTypeCommunityRule()) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG002,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_SCHEME_TYPE_COMMUNITY_RULES }));
		}
	...
	protected void checkSchemeTypeCommunityURIValue(URI uri) throws TSLMalformedException {
		...
		if (isTSLTypeEUlistOfTheList(tslType)) {
			...
			if (!valueOK) {
				if (uriString.startsWith(ITSLCommonURIs.TSL_SCHEMECOMUNITYRULES_CC_PREFFIX)
						&& uriString.length() > ITSLCommonURIs.TSL_SCHEMECOMUNITYRULES_CC_PREFFIX.length()) {
					valueOK = uriString.substring(ITSLCommonURIs.TSL_SCHEMECOMUNITYRULES_CC_PREFFIX.length())
							.equals(getTSLObject().getSchemeInformation().getSchemeTerritory());
				}
			}
	...
	protected void checkSchemeInformationTerritoryValue() throws TSLMalformedException {
		...
		if (!UtilsCountryLanguage.checkCountryCode(getTSLObject().getSchemeInformation().getSchemeTerritory())) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG003,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_SCHEME_TERRITORY }));
		}
	...
	protected void checkSchemeInformationPolicyValues() throws TSLMalformedException {
		...
		if (getTSLObject().getSchemeInformation().getPolicyInLanguage(Locale.UK.getLanguage()) == null) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG005,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_POLICY_OR_LEGAL_NOTICE }));
		}
	...
	protected void checkSchemeInformationHistoricalInformationPeriodValue() throws TSLMalformedException {
		...
		if (getTSLObject().getSchemeInformation().getHistoricalPeriod() != NumberConstants.INT_65535) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG003,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_HISTORICAL_INFORMATION_PERIOD }));
		}
	...
	protected void checkSchemeInformationPointersToOtherTSLValue() throws TSLMalformedException {
		...
		if ((isTSLTypeEUGeneric(tslType) || isTSLTypeEUlistOfTheList(tslType))
				&& !getTSLObject().getSchemeInformation().isThereSomePointerToOtherTSL()) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG002,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_POINTER_TO_OTHER_TSL_TSLLOCATION }));
		}
		...
		if (isTSLTypeEUGeneric(tslType) || isTSLTypeEUlistOfTheList(tslType)) {
			...
			for (TSLPointer tslPointer : tslPointers) {
				...
				if (TSLManager.getInstance().getSetOfURLStringThatRepresentsEuLOTL()
						.contains(tslPointer.getTSLLocation().toString())) {
			...
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG007,
					new Object[] { TSLManager.getInstance().getSetOfURLStringThatRepresentsEuLOTLinString() }));
			...
			for (ServiceDigitalIdentity sdi : sdiList) {
				if (sdi.isThereSomeIdentity()) {
					try {
						...
						throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG003,
								new Object[] { ITSLElementsAndAttributes.ELEMENT_POINTER_TO_OTHER_TSL }), e);
					}
				}
			}
		}
	 ...
	 * 
	 * @param digitalIdsList
	 *            Digital identities list to check.
	 * @throws TSLMalformedException
	 *             In case of some data has not a correct value.
	 */
	...
	private void checkDigitalID(DigitalID digitalId) throws TSLMalformedException {
		...
		if (digitalId == null) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG002,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_DIGITAL_IDENTITY }));
		} else {
	 ...
	 * @throws TSLMalformedException
	 *             If the input certificate has not a valid value.
	 */
	 ...
	 *            X509 Subject Name to check.
	 * @throws TSLMalformedException
	 *             If the input X509 Subject Name has not a valid value.
	 */
	 ...
	 *            Key Value to check.
	 * @throws TSLMalformedException
	 *             If the input Key Value has not a valid value.
	 */
	 ...
	 *            X509 SKI to check.
	 * @throws TSLMalformedException
	 *             If the input X509 SKI has not a valid value.
	 */
	...
	protected void checkSchemeInformationNextUpdateValue() throws TSLMalformedException {
		...
		if (!TimeZone.getDefault().inDaylightTime(issueDate) && TimeZone.getDefault().inDaylightTime(issueDateMod)) {
			issueDateMod = new Date(
					issueDateMod.getTime() + Integer.valueOf(TimeZone.getDefault().getDSTSavings()).longValue());
		} else if (TimeZone.getDefault().inDaylightTime(issueDate)
				&& !TimeZone.getDefault().inDaylightTime(issueDateMod)) {
			issueDateMod = new Date(
					issueDateMod.getTime() - Integer.valueOf(TimeZone.getDefault().getDSTSavings()).longValue());
		}
	...
	protected void checkSchemeInformationExtensionsDependingOnTSLType() throws TSLMalformedException {
		...
		if ((isTSLTypeEUGeneric(tslType) || isTSLTypeEUlistOfTheList(tslType))
				&& getTSLObject().getSchemeInformation().isThereSomeSchemeInformationExtension()) {
	...
	protected void checkTSPInformationNamesValues(TSPInformation tspInformation) throws TSLMalformedException {
		...
		if (namesList == null) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG005,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_TSPINFORMATION_NAME }));
		}
	...
	protected void checkTSPInformationTradeNamesValues(TSPInformation tspInformation) throws TSLMalformedException {
		...
		if (!tspInformation.isThereSomeTradeName()) {
			...
			// comprobación.
			LOGGER.warn(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG002,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_TSPINFORMATION_TRADENAME }));
		} else {
				...
				for (String tradeName : tradeNamesList) {
					...
					if (validTradeName) {
						...
						if (!officialRegistrationIdentifierFinded) {
							...
							officialRegistrationIdentifierFinded = checkIfTradeNameIsOfficialRegistrationIdentifier(
									tradeName);
			...
			if (!officialRegistrationIdentifierFinded) {
				...
				LOGGER.warn(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG008,
						new Object[] { tspInformation.getTSPNamesForLanguage(Locale.UK.getLanguage()),
								ITSLElementsAndAttributes.ELEMENT_TSPINFORMATION_TRADENAME }));
			}
	 ...
	 * @param tradeName
	 *            Trade Name to check.
	 * @return <code>true</code> if the input trade name is a official
	 *         registration identifier, otherwise <code>false</code>.
	 */
	private boolean checkIfTradeNameIsOfficialRegistrationIdentifier(String tradeName) {
		...
		if (tradeName.length() > NumberConstants.INT_6) {
			...
			// Comprobamos COD.
			result = tradeName.startsWith(ITSLOtherConstants.TOKEN_TSP_INF_TRADE_NAME_VAT)
					|| tradeName.startsWith(ITSLOtherConstants.TOKEN_TSP_INF_TRADE_NAME_NTR);
			result = result || tradeName.startsWith(ITSLOtherConstants.TOKEN_TSP_INF_TRADE_NAME_PAS)
					|| tradeName.startsWith(ITSLOtherConstants.TOKEN_TSP_INF_TRADE_NAME_IDC);
			result = result || tradeName.startsWith(ITSLOtherConstants.TOKEN_TSP_INF_TRADE_NAME_PNO)
					|| tradeName.startsWith(ITSLOtherConstants.TOKEN_TSP_INF_TRADE_NAME_TIN);
	...
	protected void checkTSPInformationURIvalues(TSPInformation tspInformation) throws TSLMalformedException {
		...
		if (uriList == null) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG005,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_TSPINFORMATION_URI }));
		}
	 ...
	 */
	@Override
	protected void checkTSPInformationExtensionsDependingOnTSLType(TSPInformation tspInformation)
			throws TSLMalformedException {
	 ...
	 */
	@Override
	protected void checkTSPServiceInformationType(ServiceHistoryInstance tspServiceInformation)
			throws TSLMalformedException {
		...
		if (serviceTypeIdentifier == null) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG002,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_TSPSERVICE_INFORMATION_TYPE }));
		}
		...
		if (isTSLTypeEUGeneric(tslTypeUriString)) {
			...
			if (valueOK && sti.equals(ITSLCommonURIs.TSL_SERVICETYPE_CA_QC)
					&& !checkIfIsDefinedSomeExtensionTypeInTSPService(tspServiceInformation,
				...
				// rechazadas.
				LOGGER.warn(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG010,
						new Object[] { tspServiceInformation.getServiceNameInLanguage(Locale.UK.getLanguage()) }));
			}
	...
	/**
	 * Auxiliar method that checks if the input Service Type Identifier is for
	 * Qualified Certificates.
	 * 
	 * @param uriStringServiceType
	 *            Service Type Identifier to analyze.
	 * @return <code>true</code> if the input Service Type Identifier is for
	 *         Qualified Certificates.
	 */
	private boolean isServiceTypeForQualifiedCerts(String uriStringServiceType) {
		...
		if (!UtilsStringChar.isNullOrEmptyTrim(uriStringServiceType)) {
			...
			result = uriStringServiceType.equals(ITSLCommonURIs.TSL_SERVICETYPE_CA_QC)
					|| uriStringServiceType.equals(ITSLCommonURIs.TSL_SERVICETYPE_CERTSTATUS_OCSP_QC)
					|| uriStringServiceType.equals(ITSLCommonURIs.TSL_SERVICETYPE_CERTSTATUS_CRL_QC);
			result = result || uriStringServiceType.equals(ITSLCommonURIs.TSL_SERVICETYPE_TSA_QTST)
					|| uriStringServiceType.equals(ITSLCommonURIs.TSL_SERVICETYPE_EDS_Q);
			result = result || uriStringServiceType.equals(ITSLCommonURIs.TSL_SERVICETYPE_EDS_REM_Q)
					|| uriStringServiceType.equals(ITSLCommonURIs.TSL_SERVICETYPE_PSES_Q);
	...
	/**
	 * Attribute that represents a URI set that defines the service types for
	 ...
	 * @param uriStringServiceType
	 *            Service Type Identifier to analyze.
	 * @return <code>true</code> if the input Service Type Identifier is for Non
	 *         Qualified Certificates.
	 */
	private boolean isServiceTypeForNonQualifiedCerts(String uriStringServiceType) {

		return !UtilsStringChar.isNullOrEmptyTrim(uriStringServiceType)
				&& serviceTypesForNonQualifiedCertsSet.contains(uriStringServiceType);
	...
	private boolean isServiceTypeForNationalCerts(String uriStringServiceType) {
		...
		return !UtilsStringChar.isNullOrEmptyTrim(uriStringServiceType)
				&& serviceTypesForNationalCertsSet.contains(uriStringServiceType);
	 ...
	 */
	@Override
	protected void checkTSPServiceInformationNamesValues(ServiceHistoryInstance tspServiceInformation)
			throws TSLMalformedException {
		...
		if (name == null) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG005,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_TSPSERVICE_INFORMATION_NAMES }));
		}
	 ...
	 */
	@Override
	protected void checkTSPServiceInformationIdentitiesSize(ServiceInformation tspServiceInformation)
			throws TSLMalformedException {
		...
		if (!tspServiceInformation.isThereSomeIdentity()) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG003,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_TSPSERVICE_INFORMATION_SERVICEDIGITALIDENTITY }));
		}
	 ...
	 */
	@Override
	protected void checkTSPServiceInformationIdentitiesValues(ServiceHistoryInstance tspServiceInformation)
			throws TSLMalformedException {
		...
		if (checkIfServiceTypeIsNoPKIorUnspecified(serviceType)) {
			...
			try {
				...
				throw new TSLMalformedException(
						Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG003,
								new Object[] {
	...
	protected boolean checkIfServiceTypeIsNoPKIorUnspecified(String serviceType) {
		...
		result = serviceType.equals(ITSLCommonURIs.TSL_SERVICETYPE_RA_NOTHAVINGPKIID)
				|| serviceType.equals(ITSLCommonURIs.TSL_SERVICETYPE_ARCHIV_NOTHAVINGPKIID)
				|| serviceType.equals(ITSLCommonURIs.TSL_SERVICETYPE_IDV_NOTHAVINGPKIID);
		result = result || serviceType.equals(ITSLCommonURIs.TSL_SERVICETYPE_KESCROW_NOTHAVINGPKIID)
				|| serviceType.equals(ITSLCommonURIs.TSL_SERVICETYPE_PPWD_NOTHAVINGPKIID);
	 ...
	 */
	@Override
	protected void checkTSPServiceInformationStatusValue(ServiceHistoryInstance tspServiceInformation)
			throws TSLMalformedException {
		...
		if (!isValid) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG003,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_TSPSERVICE_INFORMATION_STATUS }));
		}

	}

	/**
	 * Checks if the input service type and status are valid for a TSL type
	 * EU-Generic.
	 * 
	 * @param serviceType
	 *            Service type to check.
	 * @param serviceStatus
	 *            Service status to check.
	 * @return <code>true</code> if the input service type and status are valid
	 *         for a TSL type EU-Generic, otherwise <code>false</code>.
	 */
	private boolean checkIfServiceTypeIsValidForTslTypeEuGeneric(String serviceType, String serviceStatus) {
		...
		// Si es para no cualificados o nacionales...
			...
			result = serviceStatus.equals(ITSLCommonURIs.TSL_SERVICECURRENTSTATUS_RECOGNISEDATNATIONALLEVEL)
					|| serviceStatus.equals(ITSLCommonURIs.TSL_SERVICECURRENTSTATUS_DEPRECATEDATNATIONALLEVEL);
			...
			if (Calendar.getInstance().getTime().before(getInitialDate())) {
				result = result || serviceStatus.equals(ITSLCommonURIs.TSL_SERVICECURRENTSTATUS_SETBYNATIONALLAW)
						|| serviceStatus.equals(ITSLCommonURIs.TSL_SERVICECURRENTSTATUS_DEPRECATEDBYNATIONALLAW);
			}
	...
	/**
	 * Attribute that represents a set with the URI that represents the service
	 * status for a TSL EU-Generic Serivice Type for qualified certificates.
	 */
	...
	static {
		...
		serviceStatusTSLEUGenericServiceTypeQualifiedCertBeforeInitialDateSet
				.add(ITSLCommonURIs.TSL_SERVICECURRENTSTATUS_UNDERSUPERVISION);
		serviceStatusTSLEUGenericServiceTypeQualifiedCertBeforeInitialDateSet
				.add(ITSLCommonURIs.TSL_SERVICECURRENTSTATUS_SUPERVISIONINCESSATION);
		serviceStatusTSLEUGenericServiceTypeQualifiedCertBeforeInitialDateSet
				.add(ITSLCommonURIs.TSL_SERVICECURRENTSTATUS_SUPERVISIONCEASED);
		serviceStatusTSLEUGenericServiceTypeQualifiedCertBeforeInitialDateSet
				.add(ITSLCommonURIs.TSL_SERVICECURRENTSTATUS_SUPERVISIONREVOKED);
		serviceStatusTSLEUGenericServiceTypeQualifiedCertBeforeInitialDateSet
				.add(ITSLCommonURIs.TSL_SERVICECURRENTSTATUS_ACCREDITED);
		serviceStatusTSLEUGenericServiceTypeQualifiedCertBeforeInitialDateSet
				.add(ITSLCommonURIs.TSL_SERVICECURRENTSTATUS_ACCREDITATIONCEASED);
		serviceStatusTSLEUGenericServiceTypeQualifiedCertBeforeInitialDateSet
				.add(ITSLCommonURIs.TSL_SERVICECURRENTSTATUS_ACCREDITATIONREVOKED);
	}

	/**
	 * Checks if the input service status is valid for a service type for
	 * qualified certificates.
	 * 
	 * @param serviceStatus
	 *            Service status to check.
	 * @return <code>true</code> if the input service status is valid for a
	 *         service type for qualified certificates, otherwise
	 *         <code>false</code>.
	 */
	private boolean checkIfServiceStatusIsForServiceTypeForQualifiedCert(String serviceStatus) {
		...
		if (Calendar.getInstance().getTime().before(getInitialDate())) {
			...
			result = result
					|| serviceStatusTSLEUGenericServiceTypeQualifiedCertBeforeInitialDateSet.contains(serviceStatus);
	 ...
	 */
	@Override
	protected void checkTSPServiceHistoryInstanceType(ServiceHistoryInstance serviceHistoryInstance)
			throws TSLMalformedException {
		...
		if (isTSLTypeEUGeneric(tslTypeUriString)) {
			...
			if (valueOK && sti.equals(ITSLCommonURIs.TSL_SERVICETYPE_CA_QC)
				...
				LOGGER.warn(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG010,
						new Object[] { serviceHistoryInstance.getServiceNameInLanguage(Locale.UK.getLanguage()) }));
		...
		if (!valueOK) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG003,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_TSPSERVICE_HISTORY_TYPE }));
		}
	 ...
	 */
	@Override
	protected void checkTSPServiceHistoryInstanceNamesValues(ServiceHistoryInstance serviceHistoryInstance)
			throws TSLMalformedException {
	 ...
	 */
	@Override
	protected void checkTSPServiceHistoryInstanceIdentitiesValues(ServiceHistoryInstance serviceHistoryInstance)
			throws TSLMalformedException {
		...
		try {
			...
			if (checkIfServiceTypeIsNoPKIorUnspecified(serviceType)) {
				...
				try {
					...
					// del todo correcto.
					LOGGER.warn(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG003, new Object[] {
							ITSLElementsAndAttributes.ELEMENT_TSPSERVICE_INFORMATION_SERVICEDIGITALIDENTITY }));
			...
			String msg = Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG003,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_TSPSERVICE_HISTORY_SERVICEDIGITALIDENTITY });
			e.setErrorDesc(msg);
			throw e;
		}
	...
	/**
	 * Checks the Digital Ids List if it is not <code>null</code> or empty. In
	 * this list must be at least a X509SKI digital identity and not must be a
	 * X509Certificate digital identity.
	 * 
	 * @param digitalIdsList
	 *            Digital identities list to check.
	 * @throws TSLMalformedException
	 *             In case of some data has not a correct value or there is a
	 *             X509Certificate digital identity or there is not a X509SKI
	 *             digital identity
	 */
	private void checkDigitalIdsListInServiceHistoryInstance(List<DigitalID> digitalIdsList)
			throws TSLMalformedException {
	...
	/**
	 * Attribute that represents a set with the URI that defines the service
	 * status valid values for qualified certificates.
	 */
	private static Set<String> serviceStatusForQualifiedCertificates = null;

	/**
	 * Attribute that represents a set with the URI that defines the service
	 * status valid values for non qualified certificates and national
	 * certificates.
	 */
	...
	static {
		...
		serviceStatusForNonQualifiedCertificatesOrNationalCertificates
				.add(ITSLCommonURIs.TSL_SERVICECURRENTSTATUS_RECOGNISEDATNATIONALLEVEL);
		serviceStatusForNonQualifiedCertificatesOrNationalCertificates
				.add(ITSLCommonURIs.TSL_SERVICECURRENTSTATUS_DEPRECATEDATNATIONALLEVEL);
		serviceStatusForNonQualifiedCertificatesOrNationalCertificates
				.add(ITSLCommonURIs.TSL_SERVICECURRENTSTATUS_SETBYNATIONALLAW);
		serviceStatusForNonQualifiedCertificatesOrNationalCertificates
				.add(ITSLCommonURIs.TSL_SERVICECURRENTSTATUS_DEPRECATEDBYNATIONALLAW);
	}
	 ...
	 */
	@Override
	protected void checkTSPServiceHistoryInstanceStatusValue(ServiceHistoryInstance serviceHistoryInstance)
			throws TSLMalformedException {
		...
		if (!isValid) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG003,
					new Object[] { ITSLElementsAndAttributes.ELEMENT_TSPSERVICE_HISTORY_STATUS }));
	...
	protected void tslSignatureIsNotDefined() throws TSLMalformedException {
		...
		throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG002,
				new Object[] { ITSLElementsAndAttributes.ELEMENT_SIGNATURE }));
	...
	protected void checkSignatureAccordingToSpecification(byte[] fullTSLxml) throws TSLMalformedException {
		...
		if (signatureValue == null || signatureValue.getByteArrayValue() == null) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG011,
					new Object[] { ITSLSignatureConstants.ELEMENT_SIGNATUREMETHOD }));
		}
		...
		if (signedInfo == null) {
			throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG011,
					new Object[] { ITSLSignatureConstants.ELEMENT_SIGNEDINFO }));
		} else {
			...
			for (int index = 0; !referenceFinded && index < signedInfo.sizeOfReferenceArray(); index++) {
				...
				if (UtilsStringChar.isNullOrEmpty(rt.getURI())
						|| rt.getURI().equals(UtilsStringChar.SYMBOL_PAD_STRING + tslId)) {
					...
					if (rt.isSetTransforms()) {
						...
						if (tst.sizeOfTransformArray() == 2) {
							...
							if (!ITSLSignatureConstants.URI_XMLDSIG_ENVELOPED_SIGNATURE
									.equals(tst.getTransformArray(0).getAlgorithm())) {
								throw new TSLMalformedException(
										Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG012, new Object[] { tslId,
												0, ITSLSignatureConstants.URI_XMLDSIG_ENVELOPED_SIGNATURE }));
							}

							// El segundo de los elementos debe ser
							// "http://www.w3.org/2001/10/xmlexc-c14n#".
							if (!ITSLSignatureConstants.URI_CANONICALIZATION_ALGORITHM_XMLEXC_C14N
									.equals(tst.getTransformArray(1).getAlgorithm())) {
								throw new TSLMalformedException(Language
										.getFormatResIntegraTsl(ILogTslConstant.TC_LOG012, new Object[] { tslId, 1,
												ITSLSignatureConstants.URI_CANONICALIZATION_ALGORITHM_XMLEXC_C14N }));
							}

						} else {
							// Debe haber dos elementos Transform.
							throw new TSLMalformedException(
									Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG013, new Object[] { tslId }));
						}

					} else {
						// El elemento Transforms debe estar definido.
						throw new TSLMalformedException(
								Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG014, new Object[] { tslId }));
					}
			...
			if (!referenceFinded) {
				throw new TSLMalformedException(
						Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG015, new Object[] { tslId }));
			}
			...
			if (cmt == null) {
				...
				if (!ITSLSignatureConstants.URI_CANONICALIZATION_ALGORITHM_XMLEXC_C14N.equals(cmt.getAlgorithm())) {
					throw new TSLMalformedException(Language.getFormatResIntegraTsl(ILogTslConstant.TC_LOG017,
							new Object[] { ITSLSignatureConstants.URI_CANONICALIZATION_ALGORITHM_XMLEXC_C14N }));
				}
